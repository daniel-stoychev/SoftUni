    ...
    return function (target: Object, propName: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.set = function (val:string) {
            if (val.length < minLength) {
                throw new Error(`Runtime Error: name must have a min length of ${minLength} characters`);
            }
...
function validateAge(min: number, max: number) {
    return function (target: Object, propName: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.set = function (val:number) {
            if (val < min || val > max) {
                throw new Error(`Runtime Error: age must be between ${min} and ${max}`);
            }
            originalSet?.call(this, val);
        }
        return descriptor;
    ...
    return function (target: Object, propName: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.set = function (val:string) {
            if (!val.match(pattern)) {
                throw new Error(`Runtime Error: password needs to match /^[a-zA-Z0-9]+$/g`);
            }
            originalSet?.call(this, val);
        }
        return descriptor;
